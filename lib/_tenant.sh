...
sudo su - deploy <<EOF
  cat <<'[-]EOF' > /home/deploy/crmplus/tenant/.env
...
tenant_start_pm2() {
  ...
  sudo su - deploy <<EOF
  cd /home/deploy/crmplus/tenant
  pm2 start server.js --name crmplus-tenant
  pm2 save
EOF
...
cat > /etc/nginx/sites-available/crmplus-tenant << 'END'
server {
...
ln -s /etc/nginx/sites-available/crmplus-tenant /etc/nginx/sites-enabled
EOF
